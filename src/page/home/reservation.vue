<template>
    <div class="cont">
        ...
        <div class="cont_top_btn">
            ...
            <div class="lists">
                <el-input v-model.trim="ruleForm.username" class="time-left" :placeholder="$t('reception.username2')" clearable></el-input>
        ...
        <el-table :data="tableData" stripe style="width: 100%" header-align='center'>
            ...
            <el-table-column prop="reserve_time"  :label="$t('reception.reserve_time')" width="auto" show-overflow-tooltip align='center' :formatter="dateFormat"></el-table-column>
            <el-table-column prop="type" :label="$t('reception.status2')" width="auto" show-overflow-tooltip align='center'>
            ...
            <el-table-column prop="deposit" :label="$t('reception.book_money')" width="auto" show-overflow-tooltip align='center'>
                <template slot-scope='scope'>
                    <el-button v-if="scope.row.deposit==0" size="mini" type="danger" @click="handledeposit(scope.$index, scope.row)">{{$t('reception.book_money')}}</el-button>
                    <el-tag v-else>{{scope.row.deposit}}</el-tag>
                </template>
            </el-table-column>
            <el-table-column prop="total_price" :label="$t('reception.total_price')" width="auto" show-overflow-tooltip align='center'></el-table-column>
            <el-table-column prop="staff_number" :label="$t('reception.staff_name')" width="auto" show-overflow-tooltip align='center' :formatter="workerFormat"></el-table-column>
            <el-table-column prop="checkin_time" :label="$t('reception.expect_check_time')" width="auto" show-overflow-tooltip align='center' :formatter="dateFormat"></el-table-column>
            <el-table-column prop="checkout_time" :label="$t('reception.expect_out_time')" width="auto" show-overflow-tooltip align='center' :formatter="dateFormat"></el-table-column>
            <el-table-column :label="$t('public.operate')" align="center" fixed="right" width="300px">  
                <template slot-scope="scope">
                    ...
                    <el-button size="mini" @click="handlebatch(scope.$index, scope.row)">{{$t('reception.in_hotel')}}</el-button>
                    <el-button size="mini" type="danger" @click="handleDelete(scope.$index, scope.row)" v-if="scope.row.type==3">{{$t('reception.revoke')}}</el-button>
                    <el-button size="mini" type="success" v-else-if="scope.row.type==2" @click="cashPledge(scope.$index, scope.row)">{{$t('reception.cash')}}</el-button>
                    <el-button size="mini" type="danger" @click="handleDelete(scope.$index, scope.row)" v-else-if="scope.row.type==1">{{$t('public.cancel')}}</el-button>
                </template>
            </el-table-column>
        </el-table>
        <el-pagination
        ...
        <el-dialog title="预定新增" :visible.sync="dialogFormVisible" class="dia" width="40%">
            <el-form :model="forms" status-icon :rules="rule" ref="forms" label-width="80px" class="demo-ruleForm mars">
                ...
                <el-form-item :label="$t('reception.expect_check_time')" prop="checkin_time" class="floatleft">
                    <el-date-picker
                ...
                <el-form-item :label="$t('reception.expect_out_time')" prop="checkout_time" class="floatleft">
                    <el-date-picker
                ...
                <el-form-item :label="$t('reception.source_type')" prop="source_type" class="floatleft">
                    <el-select v-model.trim="forms.source_type" placeholder="客源类型">
                        <el-option v-for="item in source_type" :key="item.index" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                ...
                <el-form-item :label="$t('reception.public_pays')" prop="public_pays" class="floatleft">
                    <el-select v-model="forms.public_pays" placeholder="公费项目">
                        <el-option v-for="(item,index) in public_pays" :key="index" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
        ...
        <el-dialog   :title="$t('public.edit')" :visible.sync="dialog_2" class="dia" width="40%">
            <el-tabs v-model="activeName" :stretch="true" type="card">
                <el-tab-pane :label="$t('public.edit')" name="0">
                    <el-form :model="form_1" status-icon :rules="rule" ref="forms" label-width="80px" class="demo-ruleForm mars">
                ...
                <el-tab-pane :label="$t('reception.reserve_info')" name="1">
                    <el-form :model="form_2" status-icon :rules="rule" ref="forms" label-width="80px" class="demo-ruleForm mars">
                    ...
                    <el-form :inline="true" v-for="(item, index) in dynamicValidateForm.rooms" :key="index">
                        ...
                        <el-form-item v-if="item.status==3">
        ...
        <el-dialog title="添加入住" :visible.sync="dialog_3" class="dia" width="60%">
            ...
            <el-tabs type="border-card">
                <el-table :data="itemInfo" stripe style="width: 100%" ref="multipleTable"  header-align='center' >
                    <el-table-column type="selection" width="55"></el-table-column>
                    <el-table-column prop="house_type" :label="$t('reception.house_type')" width="150" show-overflow-tooltip align='center'
          ...
          <el-tag v-if="scope.row.type==3" type="danger">取消</el-tag>
                    ...
                    <el-table-column label="添加" width="150" align="center">
                            <template slot-scope="scope">
                                <el-button type="primary" @click="addman(scope.$index,scope.row)">{{$t('reception.add_player')}} {{scope.$index}}</el-button>
                            </template>
                    </el-table-column>

                    <el-table-column :label="$t('public.operate')" width="150px" align="center">
                  <template slot-scope="scope">
                        <el-button
                         @click="checkinByitem(scope.row)"
                       type="primary"
                       plain
                        size="mini"
        ...
        <el-dialog title="交定金" :visible.sync="dialog_4" class="dia" width="40%">
            <el-form :model="depmoreny" status-icon :rules="rule" ref="depmoreny" label-width="80px" class="demo-ruleForm mars">
                <el-form-item :label="$t('reception.amount_due')" prop="deposit" class="floatleft">
                    <el-input v-model.trim="depmoreny.deposit"></el-input>
                </el-form-item>
                <el-form-item :label="$t('reception.payf')" prop="method" class="floatleft">
                    <el-select v-model="depmoreny.method" placeholder="选择支付方式">
                        <el-option v-for="(item,index) in methodlist" :key="index" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item :label="$t('reception.pay_project')" prop="project_type" class="floatleft">
                    <el-select v-model="depmoreny.project_type" placeholder="账务项目">
                        <el-option v-for="(item,index) in project" :key="index" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item :label="$t('reception.vip_card')" prop="member_card" class="floatleft">
                    <el-input v-model="depmoreny.member_card"></el-input>
                </el-form-item>
                <el-form-item :label="$t('reception.money_detail')" prop="entry" class="floatleft">
                    <el-select v-model="depmoreny.entry" placeholder="财务明细">
                        <el-option v-for="(item,index) in itemtype" :key="index" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                <div style="clear: both;"></div>
        ...
        <el-dialog title="交押金" :visible.sync="dialog_5" class="dia" width="40%">
            <el-form :model="Pledge" status-icon :rules="rule" ref="Pledge" label-width="80px" class="demo-ruleForm mars">
                ...
                <el-form-item :label="$t('reception.password2')" prop="password" class="floatleft">
                    <el-input v-model="Pledge.password"></el-input>
                </el-form-item>
                <div style="clear: both;"></div>
                <el-form-item>
                    <el-button @click="dialog_5= false">{{$t('public.cancel')}}</el-button>
                    <el-button type="primary"  @click="Pledge()">{{$t('public.ok')}}</el-button>
                </el-form-item>
            </el-form>
        </el-dialog>
        <el-dialog :visible.sync="dialog_6" class="dia" width="40%" >
            <el-form :inline="true" status-icon class="demo-ruleForm mars" >
                <div v-for="(itemd, index) in customers" :key="'k'+index">
                    <el-form-item :label="$t('reception.username2')" prop="username">
                        <el-input v-model.trim="itemd.username"></el-input>
                    </el-form-item>
                    <el-form-item :label="$t('reception.phone2')" prop="phone">
                ...
                <el-form-item>
                    <el-button type="primary" @click="add()">{{$t('public.ok')}}</el-button>
                </el-form-item>
            </el-form>
        </el-dialog>
    </div>
        ...
        return {
           ...
           phone:''
            ...
            rule: {
                ...
                isteam:[
                    {required: true, message: '选择是否团队', trigger: 'blur'}
                ],
            },
            rule2:{
                 username : [
    ...
    houseTypeFormat(row,column){
      ...
      this.$axios
        ...
        roomList(a,b){
            ...
            that.$axios.post(this.$baseUrl + '/reservation/getPage',para)
            .then((res) => {
        ...
        info(index,row){
            ...
            this.$axios.post(this.$baseUrl +'/reservation/info',fordata)
            .then((res) => {
        ...
        modifyEvent(index,row){
            ...
            if(this.activeName==0){
                this.$axios.post(this.$baseUrl +'/reservation/updateOne',this.form_1)
                .then((res) => {
                console.log(res)
                if (res.data.result== true) {
                    // console.log(3333)
                ...
                this.$axios.post(this.$baseUrl +'/reservation/updateList',para)
                .then((res) =>{
                console.log(res)
                if (res.data.result== true) {
                    // console.log(6666)
        ...
        batchCheckIn(){
            ...
            if(this.isteam==1){
                this.$axios.post(this.$baseUrl +'/reservation/teamCheckIn',para)
                .then((res) => {
                console.log(res)
                if (res.data.result== true) {
                    console.log('成功')
                ...
                this.$axios.post(this.$baseUrl +'/reservation/batchCheckIn',para)
                .then((res) => {
                console.log(res)
                if (res.data.result== true) {
                    console.log(res)
        ...
        handleDelete(index,row){
            ...
            if(row.type==3){
                this.$confirm('此操作将增加订单, 是否继续?', '提示', {
                    ...
                    fordata.append("id",row.id)
                    fordata.append("status",1)
                    this.$axios.post(this.$baseUrl + '/reservation/batchCancel',fordata)
                    .then((res) => {
                        if (res.data.result) {
                        this.$message({
                            message: this.$t("common."+res.data.msg),
        ...
        cashmoreny(){
            this.$axios.post(this.$baseUrl +'/registerinfo/cashPledge',this.Pledge)
                .then((res) => {
                console.log(res)
                if (res.data.result== true) {
                    // console.log('成功')
        ...
        checkinByitem(row){
         ...
         this.reservationAdd.push(info)
            ...
            //判断是不是团队入住
         if( this.isteam==1){
             this.$axios.post(this.$baseUrl +'/reservation/teamCheckIn',{reservationAdds:this.reservationAdd}  
         )
         .then(res=>{
             if(res.data.result){
              console.log(res)
         ...
         .catch(err=>{
           ...
           this.$axios.post(this.$baseUrl +'/reservation/batchCheckIn',{reservationAdds:this.reservationAdd}  
         )
         .then(res=>{
             if(res.data.result){
              console.log(res)
